# -*- coding: utf-8 -*-
"""
Created on Tue Jul  5 14:00:00 2022
...
def score_function_multid_seperate_all_dims_absorbing(X,Z,func_out=False,
                                                      C=0.001,kern='RBF',l=1,
                                                      bound_dim=0,
                                                      bounds=[None, None]):
    """
    Sparse kernel based estimation of multidimensional logarithmic gradient of 
    empirical density represented 
    by samples X for all dimensions simultaneously. For absorbing boundary.
    This is the bounded estimator with reflexion of points on the boundary.
    
    - When `funct_out == False`: computes grad-log at the sample points.
    - When `funct_out == True`: return a function for the grad log to be employed 
                                for interpolation/estimation of grad log 
    ...
    -----------
            ...
            bound_dim: int , TO DO: list of ints
                        dimension that is bounded
            bounds: ndarray, TO DO: list of ndarrays 1 for every bounded dim
                        2dim array with the appropriate bounds
                        If the dimension is bounded from one side only
                        then the respectve entry should be None
    ...
    if bounds[0] is None and bounds[1] is not None:
        vthr = bounds[1] ###voltage threshold
        vt = np.tile([[vthr], [0]], N).T
        neg0 = np.tile([[-1], [1]], N).T # multiplying with this elementwise will change the sign of dim 0 only
        X2 = np.concatenate(  (X, np.multiply(neg0,(X-vt)) + vt )  , axis=0 ) 
    ...
    if isinstance(l, (list, tuple, np.ndarray)):
       ...
       gradx_K = -grdx_K_all(X2,Z,l,multil=True) #-
       gradxK = np.zeros((X2.shape[0],Z.shape[0],dim))
       for ii in range(dim):
           gradxK[:,:,ii] = -grdx_K(X2,Z,l,multil=True,which_dim=ii+1)
       
       np.testing.assert_allclose(gradxK, gradx_K) 
    else:
        multil = False        
        K_xz = K(X2,Z,l,multil=False)         
        Ks = K(Z,Z,l,multil=False)   
        Ksinv = np.linalg.inv(Ks+ 1e-3 * np.eye(Z.shape[0]))
        A = K_xz.T @ K_xz    
        
        gradx_K = -grdx_K_all(X2,Z,l,multil=False)   #shape: (N,M,dim)
    ...
    if func_out==False: #if output wanted is evaluation at data points
        ...
        res1 = (-K_xz @ np.linalg.inv( C*np.eye(Z.shape[0], Z.shape[0]) + Ksinv @ A + 1e-3 * np.eye(Z.shape[0]))@ Ksinv@sumgradx_K  )[:N,:]       
